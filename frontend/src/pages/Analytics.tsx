  ...
  return (
    <div className="p-8 overflow-y-auto" style={{ height: 'calc(100vh - 68px)' }}>
      <div style={{ marginBottom: 32 }}>
        ...
        <p style={{ color: 'var(--uc-text-muted)', fontSize: 13, marginTop: 4 }}>Corrélations, tendances et insights prédictifs • 2023-24</p>
      </div>

      {/* Summary Cards */}
      <div className="grid grid-cols-4 gap-5" style={{ marginBottom: 28 }}>
        {[
          { label: "Score Global", value: "74.2", icon: <TrendingUp size={20} />, sub: "Indice composite" },
          { label: "Modèles IA Actifs", value: "3", icon: <LineChartIcon size={20} />, sub: "Prédiction & Clustering" },
          { label: "Alertes Prédictives", value: `${overview?.active_alerts || 0}`, icon: <BarChart3 size={20} />, sub: "Détectées automatiquement" },
          { label: "Institutions Analysées", value: `${overview?.total_institutions || 33}`, icon: <PieIcon size={20} />, sub: "Couverture complète" },
        ].map((k, i) => (
        ...
        <div className="uc-card col-span-2" style={{ padding: '24px 28px' }}>
          ...
          <ResponsiveContainer width="100%" height={280}>
            <LineChart data={performanceTrend}>
              ...
              <Line type="monotone" dataKey="employability" name="Employabilité %" stroke="#22c55e" strokeWidth={2} dot={{ r: 3 }} />
              <Line type="monotone" dataKey="satisfaction" name="Satisfaction %" stroke={isDark ? '#f0cc6e' : '#60a5fa'} strokeWidth={2} dot={{ r: 3 }} />
      ...
      <div style={{ marginTop: 36, marginBottom: 20 }}>
        ...
        <p style={{ color: 'var(--uc-text-muted)', fontSize: 12, marginTop: 4 }}>
          Modeles scikit-learn entraines sur les donnees historiques 2019-2025
        </p>
      </div>
      ...
      <div className="uc-card" style={{ padding: '24px 28px', marginBottom: 28 }}>
        <h3 style={{ fontSize: 13, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--uc-text)', marginBottom: 20 }}>
          Risque d'Abandon par Etablissement
        </h3>
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--uc-border)' }}>
                {["Etablissement", "Taux Actuel", "Predit", "Tendance", "Score", "Risque", "Confiance"].map(h => (
                  <th key={h} style={{ padding: '10px 12px', textAlign: 'left', color: 'var(--uc-text-muted)', fontWeight: 600, fontSize: 11 }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
      ...
      <div className="uc-card" style={{ padding: '24px 28px', marginBottom: 28 }}>
        <div className="flex items-center justify-between" style={{ marginBottom: 20 }}>
          <h3 style={{ fontSize: 13, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--uc-text)' }}>
            Prevision des Inscriptions 2026-2027
          </h3>
          {enrollForecast?.confidence && (
            <span style={{ fontSize: 11, color: accent, background: `${accent}15`, padding: '4px 12px', borderRadius: 20, fontWeight: 600 }}>
              Confiance: {enrollForecast.confidence}%
            </span>
          )}
        </div>
        <ResponsiveContainer width="100%" height={300}>
          <AreaChart data={[...(enrollForecast?.historical || []), ...(enrollForecast?.forecast || [])]}>
            ...
            <Tooltip contentStyle={{ background: 'var(--uc-bg-card)', border: '1px solid var(--uc-border)', borderRadius: 10, color: 'var(--uc-text)' }} />
            <Area type="monotone" dataKey="enrolled" name="Inscrits" stroke={accent} fill="url(#gradForecast)" strokeWidth={2} />
          </AreaChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="uc-card" style={{ padding: '24px 28px' }}>
        <div className="flex items-center justify-between" style={{ marginBottom: 20 }}>
          <h3 style={{ fontSize: 13, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--uc-text)' }}>
            Prevision Budgetaire 2026
          </h3>
          {budgetForecast?.confidence_allocated && (
            <span style={{ fontSize: 11, color: accent, background: `${accent}15`, padding: '4px 12px', borderRadius: 20, fontWeight: 600 }}>
              Confiance: {budgetForecast.confidence_allocated}%
            </span>
          )}
        </div>
        <ResponsiveContainer width="100%" height={300}>
          <BarChart data={[...(budgetForecast?.historical || []).slice(-6), ...(budgetForecast?.forecast || [])]}>
            ...
            <YAxis tick={{ fill: 'var(--uc-text-muted)', fontSize: 11 }} stroke="var(--uc-border)" />
            <Tooltip contentStyle={{ background: 'var(--uc-bg-card)', border: '1px solid var(--uc-border)', borderRadius: 10, color: 'var(--uc-text)' }} />
            <Bar dataKey="allocated" name="Alloue" fill={accent} radius={[4, 4, 0, 0]} barSize={20} opacity={0.7} />
            <Bar dataKey="consumed" name="Consomme" fill={isDark ? '#f0cc6e' : '#60a5fa'} radius={[4, 4, 0, 0]} barSize={20} />
            <Legend wrapperStyle={{ color: 'var(--uc-text-muted)', fontSize: 11 }} />
          </BarChart>
        </ResponsiveContainer>
      </div>
  ...
  return (
    <div className="flex items-center justify-center" style={{ height: '60vh' }}>
      <div className="flex flex-col items-center gap-4">
        ...
        <span style={{ color: 'var(--uc-text-muted)', fontSize: 14, fontWeight: 500 }}>Chargement des analyses...</span>
      </div>
