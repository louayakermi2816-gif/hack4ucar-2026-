...
export default function Enrollment() {
  ...
  const distributionData = [
    { name: t("enrollment.types.faculties"), value: 45, color: isDark ? '#D4AF37' : '#3b82f6' },
    { name: t("enrollment.types.schools"), value: 30, color: isDark ? '#f0cc6e' : '#60a5fa' },
    { name: t("enrollment.types.institutes"), value: 25, color: isDark ? '#8B7225' : '#93c5fd' },
  ];

  const kpis = [
    { label: t("enrollment.kpis.total"), value: fmt(totalEnrolled), icon: <GraduationCap size={20} />, trend: "+3.2%", up: true },
    { label: t("enrollment.kpis.success"), value: `${avgSuccess}%`, icon: <UserCheck size={20} />, trend: "+1.9%", up: true },
    { label: t("enrollment.kpis.dropout"), value: `${avgDropout}%`, icon: <TrendingDown size={20} />, trend: "-0.4%", up: false },
    { label: t("enrollment.kpis.attendance"), value: `${avgAttendance}%`, icon: <Users size={20} />, trend: "+0.8%", up: true },
  ];
  ...
  return (
    <div className="p-8 overflow-y-auto" style={{ height: 'calc(100vh - 68px)' }}>
      <div style={{ marginBottom: 32 }}>
        ...
        <p style={{ color: 'var(--uc-text-muted)', fontSize: 13, marginTop: 4 }}>{t("enrollment.subtitle")}</p>
      </div>
        ...
        {kpis.map((k, i) => (
          <div key={i} className="uc-card" style={{ padding: '22px 24px' }}>
            ...
            <div className="flex items-center gap-1" style={{ marginTop: 6 }}>
              ...
              <span style={{ fontSize: 11, color: 'var(--uc-text-muted)', marginLeft: 4 }}>{t("enrollment.kpis.vs_last_year")}</span>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-3 gap-5" style={{ marginBottom: 28 }}>
        ...
        <div className="uc-card col-span-2" style={{ padding: '24px 28px' }}>
          ...
          <ResponsiveContainer width="100%" height={280}>
            <AreaChart data={trendData}>
              ...
              <Area type="monotone" dataKey="enrolled" name={t("enrollment.charts.enrolled")} stroke={accent} fill="url(#gradUndergrad)" strokeWidth={2} />
        ...
        <div className="uc-card" style={{ padding: '24px 28px' }}>
          <h3 style={{ fontSize: 13, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--uc-text)', marginBottom: 20 }}>
            {t("enrollment.charts.distribution")}
          </h3>
          <ResponsiveContainer width="100%" height={280}>
            <PieChart>
      ...
      <div className="uc-card" style={{ padding: '24px 28px' }}>
        ...
        <ResponsiveContainer width="100%" height={320}>
          <BarChart data={ranking || []} layout="vertical" margin={{ left: 180 }}>
            ...
            <Tooltip contentStyle={{ background: 'var(--uc-bg-card)', border: '1px solid var(--uc-border)', borderRadius: 10, color: 'var(--uc-text)' }} />
            <Bar dataKey="value" name={t("enrollment.charts.success_rate")} fill={accent} radius={[0, 6, 6, 0]} barSize={18} />
          </BarChart>
        </ResponsiveContainer>
      </div>
    </div>
  ...
  return (
    <div className="flex items-center justify-center" style={{ height: '60vh' }}>
      <div className="flex flex-col items-center gap-4">
        ...
        <span style={{ color: 'var(--uc-text-muted)', fontSize: 14, fontWeight: 500 }}>{t("enrollment.loading") || "Chargement..."}</span>
      </div>
