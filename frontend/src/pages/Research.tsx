...
export default function Research() {
  ...
  const kpis = [
    { label: "Publications", value: `${totalPub}`, icon: <BookOpen size={20} />, trend: "+11.4%", up: true },
    { label: "Brevets Déposés", value: `${totalPatents}`, icon: <Lightbulb size={20} />, trend: "+5.2%", up: true },
    { label: "Financement Obtenu", value: `${fmt(fundingSecured)} TND`, icon: <FileText size={20} />, trend: "+8.1%", up: true },
    { label: "Projets Actifs", value: "142", icon: <Microscope size={20} />, trend: "+3.6%", up: true },
  ];

  const fundingBySource = [
    { name: "Ministère", value: 40, color: isDark ? '#D4AF37' : '#3b82f6' },
    { name: "International", value: 28, color: isDark ? '#f0cc6e' : '#60a5fa' },
    { name: "Privé", value: 18, color: isDark ? '#8B7225' : '#93c5fd' },
    { name: "UE Horizon", value: 14, color: isDark ? '#C9A636' : '#2563eb' },
  ];
  ...
  return (
    <div className="p-8 overflow-y-auto" style={{ height: 'calc(100vh - 68px)' }}>
      <div style={{ marginBottom: 32 }}>
        ...
        <p style={{ color: 'var(--uc-text-muted)', fontSize: 13, marginTop: 4 }}>Production scientifique et financement • 2023-24</p>
      </div>
        ...
        <div className="uc-card col-span-2" style={{ padding: '24px 28px' }}>
          ...
          <ResponsiveContainer width="100%" height={280}>
            <AreaChart data={yearlyTrend}>
              ...
              <Area type="monotone" dataKey="publications" name="Publications" stroke={accent} fill="url(#gradPub)" strokeWidth={2.5} />
        ...
        <div className="uc-card" style={{ padding: '24px 28px' }}>
          ...
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={rankPub || []} layout="vertical" margin={{ left: 140 }}>
              ...
              <Tooltip contentStyle={{ background: 'var(--uc-bg-card)', border: '1px solid var(--uc-border)', borderRadius: 10, color: 'var(--uc-text)' }} />
              <Bar dataKey="value" name="Publications" fill={accent} radius={[0, 6, 6, 0]} barSize={16} />
            </BarChart>
          </ResponsiveContainer>
        </div>
        <div className="uc-card" style={{ padding: '24px 28px' }}>
          <h3 style={{ fontSize: 13, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: 'var(--uc-text)', marginBottom: 20 }}>
            Brevets par Établissement
          </h3>
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={rankPatent || []} layout="vertical" margin={{ left: 140 }}>
              ...
              <YAxis type="category" dataKey="institution" tick={{ fill: 'var(--uc-text-muted)', fontSize: 9 }} stroke="var(--uc-border)" width={135} />
              <Tooltip contentStyle={{ background: 'var(--uc-bg-card)', border: '1px solid var(--uc-border)', borderRadius: 10, color: 'var(--uc-text)' }} />
              <Bar dataKey="value" name="Brevets" fill={isDark ? '#f0cc6e' : '#60a5fa'} radius={[0, 6, 6, 0]} barSize={16} />
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
  ...
  return (
    <div className="flex items-center justify-center" style={{ height: '60vh' }}>
      <div className="flex flex-col items-center gap-4">
        ...
        <span style={{ color: 'var(--uc-text-muted)', fontSize: 14, fontWeight: 500 }}>Chargement de la recherche...</span>
      </div>
