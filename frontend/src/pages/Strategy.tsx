...
export default function Strategy() {
  ...
  const objectives = [
    { name: "Excellence Académique", progress: 78, status: "on-track", target: "Taux réussite > 80%" },
    { name: "Recherche & Innovation", progress: 65, status: "at-risk", target: "500+ publications/an" },
    { name: "Transformation Digitale", progress: 42, status: "delayed", target: "100% plateformes en ligne" },
    { name: "Partenariats Internationaux", progress: 88, status: "on-track", target: "50+ accords actifs" },
    { name: "Développement Durable", progress: 55, status: "at-risk", target: "30% réduction carbone" },
    { name: "Gouvernance & Qualité", progress: 72, status: "on-track", target: "Certification ISO" },
  ];

  const radarData = [
    { axis: "Académique", value: 78 },
    { axis: "Recherche", value: 65 },
    { axis: "Digital", value: 42 },
    { axis: "International", value: 88 },
    { axis: "Durable", value: 55 },
    { axis: "Qualité", value: 72 },
  ];
  ...
  const statusLabel = (s: string) => s === "on-track" ? "En bonne voie" : s === "at-risk" ? "À surveiller" : "En retard";
  ...
  return (
    <div className="p-8 overflow-y-auto" style={{ height: 'calc(100vh - 68px)' }}>
      <div style={{ marginBottom: 32 }}>
        ...
        <p style={{ color: 'var(--uc-text-muted)', fontSize: 13, marginTop: 4 }}>Suivi des objectifs stratégiques • Horizon 2025</p>
      </div>

      {/* Summary KPIs */}
      <div className="grid grid-cols-3 gap-5" style={{ marginBottom: 28 }}>
        {[
          { label: "En Bonne Voie", count: objectives.filter(o => o.status === "on-track").length, color: "#22c55e", icon: <CheckCircle2 size={20} /> },
          { label: "À Surveiller", count: objectives.filter(o => o.status === "at-risk").length, color: isDark ? '#D4AF37' : '#f59e0b', icon: <Clock size={20} /> },
          { label: "En Retard", count: objectives.filter(o => o.status === "delayed").length, color: "#ef4444", icon: <AlertCircle size={20} /> },
        ].map((s, i) => (
          ...
          <div className="flex flex-col gap-4">
            {objectives.map((obj, i) => (
              <div key={i} style={{ padding: '16px 20px', background: 'var(--uc-bg)', borderRadius: 12, border: '1px solid var(--uc-border)' }}>
                ...
                <div style={{ fontSize: 11, color: 'var(--uc-text-muted)', marginBottom: 8 }}>Cible: {obj.target}</div>
